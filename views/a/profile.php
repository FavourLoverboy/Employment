    ...
    <?php
        ...
        if(isset($_POST['details'])){
            ...
            if($validate_email){
                ...
                if($continue){
                    ...
                    if($update){
                        $e = $n = $errEmail = '';
                        echo "<script>  window.location='a/profile/success' </script>";
                    }
                }
            }else{
        ...
        if(isset($_POST['image'])){
            ...
            if(in_array($ext, $pictureAllowed)){
                $tblquery = "UPDATE admins SET img = :img WHERE id = :id";
                $tblvalue = [
                ...
                if($update){
                    if($_SESSION['img']){
                        ...
                        if(file_exists("uploads/admins/$_SESSION[myID]/$delFile")){
                            unlink("uploads/admins/$_SESSION[myID]/$delFile");
                        }
                    }

                    if(!is_dir("uploads/admins/$_SESSION[myID]")){
                        mkdir("uploads/admins/$_SESSION[myID]", 0777, true);
                    }
                    $location = "uploads/admins/$_SESSION[myID]/$name";
                        ...
                        <div class="form-group">
                            <?php 
                                ...
                                if($_SESSION['img']){
                                    $imgFile = $secObj->decryptURLParam($_SESSION['img']);
                                    echo "
                                        <img src='uploads/admins/$_SESSION[myID]/$imgFile' alt='User Image' style='border-radius: 50%; height: 50px; width: 50px;' class='mb-2' id='output'/>
                                    ";
                                }else{
                                    echo "
